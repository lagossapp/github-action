...
Easily integrate [Lagoss](https://lagoss.com) CLI into your Github workflows. Deploy new functions, retrieve a list of existing functions, promote functions, etc. This action supports any [arbitrary input](#other-commands) of what to do!
...
_NOTE: Make sure the repository that gets checked out contains a `.lagoss/config.json` file that specifies information such as the function_id and organization_id or pass in a config via the config input value mentioned below!_
...
```bash
        with:
          ...
          config: |
            {
              "function_id": "${{ vars.lagoss_function_id }}",
